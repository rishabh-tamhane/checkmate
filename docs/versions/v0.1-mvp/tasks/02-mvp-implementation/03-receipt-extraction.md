...
- [x] **M3-01:** Add `openai`, `pillow`, and `python-multipart` with `uv add`;
...
- [x] **M3-02:** Confirm the OpenAI SDK, Pillow, and multipart parser remain
  outside the domain package and no OCR, storage, or background-job dependency
  is added.

## Application-owned extraction contract

- [x] **M3-03:** Define immutable `NormalizedReceiptImage` with JPEG bytes,
  dimensions, and media type.
- [x] **M3-04:** Define `ExtractionResult` with ordered editable item drafts,
  optional restaurant/date/totals strings, and non-blocking review notices.
- [x] **M3-05:** Define asynchronous vendor-neutral `ReceiptParser.parse` and a
  deterministic `FakeReceiptParser` for tests and local demonstrations.
- [x] **M3-06:** Prevent SDK response objects, provider IDs, prompts, and token
  accounting from crossing into application, domain, or web models.

## Bounded upload and image normalization

- [x] **M3-07:** Accept one multipart field named `receipt` and reject missing,
...
- [x] **M3-14:** Implement the private strict Pydantic provider schema with
  ...
  limits.
- [x] **M3-15:** Add a separately versioned extraction prompt whose only job is
  transcription and which treats all image text as untrusted receipt content.
- [x] **M3-16:** Call the Responses API with model snapshot
...
- [x] **M3-28:** Generate synthetic images to unit-test each accepted format,
...
- [x] **M3-29:** Verify normalized JPEG dimensions, orientation, quality path,
  and absence of source metadata without committing real receipt images.
- [x] **M3-30:** Contract-test the OpenAI adapter with synthetic SDK response
...
- [x] **M3-31:** Application-test fake-parser success and every failure category
  without importing provider objects outside the adapter.
- [x] **M3-32:** HTTP-test multipart bounds, content detection, status mapping,
  response schema, origin policy, cache policy, cleanup, and privacy-safe logs.
- [x] **M3-33:** Browser-test successful upload, editable correction,
  participant retention, assignment clearing, failure preservation, retry, and
  no-key manual mode using the fake parser.
- [x] **M3-34:** Create 12 generated external-evaluation receipts covering
...
- [x] Acceptance criteria 1, 2, and 6 pass with the deterministic fake parser.
- [ ] The recorded external evaluation meets every approved threshold.
- [x] Unsupported or hostile uploads cannot bypass byte, pixel, format,
  animation, or metadata controls.
- [x] Provider failure never loses the user's draft or disables manual entry.
- [x] Normal automated tests contain only generated receipts and fictional data.
